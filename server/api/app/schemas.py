...
class Token(BaseModel):
    ...
    directions: list[str] = []
    # Разделы приложения, закрытые этой учётке. Клиент по ним убирает вкладки
    # из меню — это настройка рабочего места, а не право на данные: доступ к
    # общей базе сервер считает сам и присылает готовым в `editable`.
    denied_pages: list[str] = []
...
class UserIn(BaseModel):
    ...
    directions: list[str] = []
    # Коды разделов, которые человеку не показывать. Коды приходят от
    # приложения и здесь не проверяются: их список живёт в клиенте и меняется
    # с каждой версией, а справочник на сервере пришлось бы догонять миграцией.
    denied_pages: list[str] = []
    is_admin: bool = False
    is_active: bool = True
