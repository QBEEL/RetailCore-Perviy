...
def _natural(text: str) -> tuple:
    ...
    Цифра проверяется через `isdecimal`, а не `isdigit`: последний считает
    цифрой и подстрочную «₃» из названия «BlanX O₃X», и число из неё уже не
    получается — весь отчёт падал на одной такой номенклатуре. Такие знаки
    сортируются как обычные символы.
    """
